...
export default function FeedPage() {
  ...
  // Load materials from following
  useEffect(() => {
    if (!user || !isFirebaseConfigured || !db) return

    const database = db // TypeScript narrowing
    ...
    const loadFollowingMaterials = async () => {
      try {
        ...
        const q = query(
          ...
          where('authorId', 'in', following.slice(0, 10)), // Firestore limit
          where('isPublic', '==', true),
          orderBy('createdAt', 'desc'),
          limit(30)
        )
  ...
  return (
    <div className="container mx-auto py-6 px-4">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Лента материалов</h1>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="following">
          {followingMaterials.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              <p className="mb-2">Нет материалов от ваших подписок</p>
              <p className="text-sm">Подпишитесь на интересных авторов, чтобы видеть их материалы здесь</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
